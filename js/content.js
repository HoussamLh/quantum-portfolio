  ...
  root.innerHTML = Object.entries(grouped)
    .map(([section, list], idx) => {
      ...
      <div class="service-card reveal active">
        ...
        ${
          s.contactService
            ? `<a href="contact.html?service=${encodeURIComponent(s.contactService)}" class="service-link">Start Project →</a>`
            : ""
        }
      </div>
    `
      ...
      return `<h3 class="section-title reveal active">0${idx + 1}. ${escapeHtml(section)}</h3><div class="services-grid">${cards}</div>`;
    })
    .join("");
}
...
async function renderProjects() {
  ...
  grid.innerHTML = items
    .map((p) => {
      ...
      return `
      <div class="project-card reveal active ${escapeHtml(p.filter)}" onclick="showProject(this, true)"
